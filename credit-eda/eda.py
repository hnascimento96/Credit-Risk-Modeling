...
def default_rate_plots(credit_data):
...
def missing_values_statistics(credit_data):
     print("-"*150)
     print("Statistical analysis of missing values by column")

     columns = credit_data.columns.to_list()

     for column in columns:

          
          print('Statistics for non-missing values:')
          mask = ~credit_data[column].isna()
          print(f"{column}:{credit_data[mask]['Default'].value_counts(normalize=True)}")


          #Gets a DF for missing values
          mask = credit_data[column].isna()
          missed_df = credit_data[mask]

          print('Statistics for missing values:')
          print(f"{column}:{missed_df['Default'].value_counts(normalize=True)}")
...
#default_rate_plots(credit_data)

missing_values_statistics(credit_data)
